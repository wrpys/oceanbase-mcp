...
async function getPrimaryKey(adapter: DatabaseAdapter, table: string, database?: string): Promise<string | null> {
  ...
  const safeTable = table.toUpperCase();
  ...
  // Oracle 模式查询主键
  const oracleSql = `SELECT cols.column_name FROM user_constraints cons, user_cons_columns cols WHERE cons.constraint_type = 'P' AND cons.table_name = '${safeTable}' AND cons.constraint_name = cols.constraint_name`;

  // MySQL 模式查询主键
  const mysqlSql = `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_NAME = '${safeTable}' AND CONSTRAINT_NAME = 'PRIMARY'`;
...
function registerTools(
  ...
  server.tool(
    ...
    async (params: { table: string; data: Record<string, unknown> | Record<string, unknown>[]; confirm?: boolean }) => {
  ...
  server.tool(
    ...
    async (params: { table: string; where?: string; id?: string | number; ids?: (string | number)[]; confirm?: boolean }) => {
  ...
  server.tool(
    ...
    async (params: { table: string; data: Record<string, unknown>; where?: string; id?: string | number; confirm?: boolean }) => {
